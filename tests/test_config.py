...
def test_get_resource(mocker):
    ...
    assert (
        get_resource("icon_file", get_full_path=False)
        == "icons/jspeak.ico.png.qrt"
    )
    assert (
        get_resource("icon_file", get_full_path=True)
        == "/anki2/addons21/jspeak/icons/jspeak.ico.png.qrt"
    )
...
def test_get_user_config(mocker):
    class MockedConfig:
        def __init__(self):
            ...
            self.addonManager.getConfig = MagicMock(
                return_value={"tree": "poop"}
            )
